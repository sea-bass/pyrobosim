...
from pyrobosim_msgs.msg import ExecutionResult, RobotState, TaskAction
...
class TestRosInterface:
    ...
    def test_start_ros_interface():
        ...
        TestRosInterface.ros_interface = WorldROSWrapper(state_pub_rate=0.1, dynamics_rate=0.01)
    ...
    @staticmethod
    @pytest.mark.dependency(name="test_robot_pub_sub", depends=["test_start_ros_interface"])    
    def test_robot_pub_sub():
        ...
        state_sub = TestRosInterface.node.create_subscription(RobotState, "robot0/robot_state", state_cb, 10)
    ...
    def test_get_world_state():
        ...
        client = TestRosInterface.node.create_client(RequestWorldState, "request_world_state")
    ...
    @staticmethod
    @pytest.mark.dependency(name="test_set_location_state", depends=["test_get_world_state"])  
    def test_set_location_state():
        """Test that we can set location states."""
        client = TestRosInterface.node.create_client(SetLocationState, "set_location_state")

        # Try to close the table, which should fail as it is locked.
        with pytest.warns(UserWarning):
            future = client.call_async(SetLocationState.Request(
    ...
    @staticmethod
    @pytest.mark.dependency(name="test_execute_action", depends=["test_set_location_state"])  
    def test_execute_action():
        """Test that we can execute a single action on a robot."""

        action_client = ActionClient(TestRosInterface.node, ExecuteTaskAction, "execute_action")
        ...
        assert result_future.result().result.execution_result.status == ExecutionResult.SUCCESS
        import pdb
        pdb.set_trace()


    # @pytest.mark.dependency(name="test_shutdown_ros_interface", depends=["test_robot_pub_sub"])    
