...
class World:
    ...
    def is_connectable(
        ...
        known_hallway_states: set[Hallway] = None,
    ) -> bool:
        """
        ...
        for x_check, y_check in zip(x_pts[1:], y_pts[1:]):
            if self.check_occupancy(Pose(x=x_check, y=y_check), partial_observability_hallway_states, known_hallway_states):
                return False
    ...
    def check_occupancy(self, pose: Pose | Sequence[float], partial_observability_hallway_states: bool = False, known_hallway_states: set[Hallway] = None) -> bool:
        """
    ...
    def is_path_collision_free(self, path: Path, step_dist: float = 0.01, partial_observability_hallway_states: bool = False, known_hallway_states: set[Hallway] = None) -> bool:
        """
        ...
        for idx in range(len(path.poses) - 1):
            if not self.is_connectable(
                path.poses[idx], path.poses[idx + 1], step_dist=step_dist, partial_observability_hallway_states=partial_observability_hallway_states, known_hallway_states=known_hallway_states
            ):
                return False
        return True

    def sample_free_robot_pose_uniform(
        self, robot: Robot | None = None, ignore_robots: bool = True, partial_observability_hallway_states: bool = False, known_hallway_states: set[Hallway] = None
    ) -> Pose | None:
        """
        ...
        for _ in range(self.max_object_sample_tries):
            ...
            if not self.check_occupancy(pose, partial_observability_hallway_states, known_hallway_states) and (
